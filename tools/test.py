...
def get_no_evaluated_ckpt(ckpt_dir, ckpt_record_file, args):
    ...
    # ckpt_list.sort(key=os.path.getmtime) # transferring via sftp modifies the original timestamp of the file
    ckpt_list= sorted(ckpt_list, key=lambda x: int(x.split('_')[-1].split('.')[0]))
